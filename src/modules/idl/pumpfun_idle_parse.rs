...
pub mod ix_discriminator {
    ...
    pub const ALL: &[(&str, [u8; 8])] = &[
        ("admin_set_creator", ADMIN_SET_CREATOR),
        ("admin_set_idl_authority", ADMIN_SET_IDL_AUTHORITY),
        ("admin_update_token_incentives", ADMIN_UPDATE_TOKEN_INCENTIVES),
        ("buy", BUY),
        ("buy_exact_sol_in", BUY_EXACT_SOL_IN),
        ("claim_cashback", CLAIM_CASHBACK),
        ("claim_token_incentives", CLAIM_TOKEN_INCENTIVES),
        ("close_user_volume_accumulator", CLOSE_USER_VOLUME_ACCUMULATOR),
        ("collect_creator_fee", COLLECT_CREATOR_FEE),
        ("create", CREATE),
        ("create_v2", CREATE_V2),
        ("distribute_creator_fees", DISTRIBUTE_CREATOR_FEES),
        ("extend_account", EXTEND_ACCOUNT),
        ("get_minimum_distributable_fee", GET_MINIMUM_DISTRIBUTABLE_FEE),
        ("init_user_volume_accumulator", INIT_USER_VOLUME_ACCUMULATOR),
        ("initialize", INITIALIZE),
        ("migrate", MIGRATE),
        ("migrate_bonding_curve_creator", MIGRATE_BONDING_CURVE_CREATOR),
        ("sell", SELL),
        ("set_creator", SET_CREATOR),
        ("set_mayhem_virtual_params", SET_MAYHEM_VIRTUAL_PARAMS),
        ("set_metaplex_creator", SET_METAPLEX_CREATOR),
        ("set_params", SET_PARAMS),
        ("set_reserved_fee_recipients", SET_RESERVED_FEE_RECIPIENTS),
        ("sync_user_volume_accumulator", SYNC_USER_VOLUME_ACCUMULATOR),
        ("toggle_cashback_enabled", TOGGLE_CASHBACK_ENABLED),
        ("toggle_create_v2", TOGGLE_CREATE_V2),
        ("toggle_mayhem_mode", TOGGLE_MAYHEM_MODE),
        ("update_global_authority", UPDATE_GLOBAL_AUTHORITY),
    ];
}
